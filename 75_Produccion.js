...
const Produccion = (() => {
  ...
  function handleUpdate_(update) {
    ...
    if (state.step === 'ask_producto') {
      ...
      saveState_(state);
      reply_(activeChatId, '🔢 ¿Cuántas piezas/kg produjiste? (solo número)');
      return;
    }
    ...
    if (state.step === 'ask_cantidad') {
      ...
      state.unidad = lookupUnidad_(state.producto);
      registerProduccion_(state);
      reply_(activeChatId, `✅ Producción registrada: ${state.categoria} → ${state.producto} (${qty}).`);
      clearState_();
      return;
    }
  }
