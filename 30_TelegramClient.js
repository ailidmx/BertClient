...
const Telegram = (() => {
  ...
  function getTelegram_() {
    return Config.getTelegram_();
  }

  function sendText_(text, opts = {}) {
    const tg = getTelegram_();
  ...
  function sendTextToTopic_(topicKey, text) {
    const threadId = getTelegram_().TOPICS[topicKey];
  ...
  function answerCallback_(callbackId, text) {
    const tg = getTelegram_();
    const url = `https://api.telegram.org/bot${tg.TOKEN}/answerCallbackQuery`;
    const payload = {
  ...
  function clearInlineButtons_(chatId, messageId) {
    const tg = getTelegram_();
    const url = `https://api.telegram.org/bot${tg.TOKEN}/editMessageReplyMarkup`;
    const payload = {
      chat_id: Number(chatId),
  ...
  function sendPhotoUrlToTopic_(topicKey, photoUrl, caption) {
    const topicId = getTelegram_().TOPICS[topicKey];
