...
const CRM = (() => {
  ...
  function runHourly_(now, opts = {}) {
    ...
    maybeSendInicioFinMesAlerts_(now, topicKey, force);
  ...
  function runApertura_(now, opts = {}) {
    ...
    const kpi = Ventas.computeKpiDiario_(now);
    const month = Calendario.getMonthTotals_(now);
    maybeSendInicioFinMesAlerts_(now, 'VENTAS', force);
  ...
  function maybeSendInicioFinMesAlerts_(now, topicKey, force) {
    if (Calendario.isFirstWorkingDay_(now)) {
      ...
      sendOnce_(key, () => {
        const cal = Calendario.getRowForDate_(now);
        const msg = CRM_Messages.buildInicioMes_(now, cal || {});
        Telegram.sendTextToTopic_(topicKey || 'VENTAS', msg);
      }, force);
    }
    ...
    if (Calendario.isLastWorkingDay_(now)) {
      ...
      sendOnce_(key, () => {
        const cal = Calendario.getRowForDate_(now);
        const msg = CRM_Messages.buildFinMes_(now, cal || {});
        Telegram.sendTextToTopic_(topicKey || 'VENTAS', msg);
      }, force);
    }
  }
